...
#   from fast_rcnn_config import model
model = __C
...
def model_from_file(filename):
  ...
  with open(filename, 'r') as f:
    yaml_model = edict(yaml.load(f))

  _merge_a_into_b(yaml_model, __C)


def model_from_list(model_list):
  """Set config keys via list (e.g., from command line)."""
  from ast import literal_eval
  assert len(model_list) % 2 == 0
  for k, v in zip(model_list[0::2], model_list[1::2]):
    key_list = k.split('.')
    d = __C
